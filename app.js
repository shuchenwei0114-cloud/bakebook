...
async function loadData() {
  const [categoriesRes, recipesRes] = await Promise.all([
    fetch("data/categories.json?v=5", { cache: "no-store" }),
    fetch("data/recipes.json?v=5", { cache: "no-store" })
  ]);
...
function normalizeRecipe(recipe) {
  ...
  if (steps.length && typeof steps[0] === "string") {
    steps = steps.map(text => ({ text, minutes: 0 }));
  }
  return {
    id: recipe.id,
...
byId("recognizeImage")?.addEventListener("click", () => {
  const file = byId("recipeImageInput").files?.[0];
  if (!file) return showMessage("先选择一张图片", "请选择视频截图、手写配方或食谱截图。" );
  showMessage("图片识别入口已经准备好", "下一步接 AI 识别接口后，这张图片会被解析成食谱名称、配料和制作步骤，并自动填入当前表单。识别不到的内容会留空，不会乱猜。" );
});
